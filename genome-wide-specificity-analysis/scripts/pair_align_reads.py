...
Pair R1 and R2, align paired reads to genome using Bowtie2 and filtering for MAPQ of 23, make bam file, sort, index
    1. Looks for R1 and R2 .fastq.gz files in "trimmed_fastq" directory and with only string difference of 
    "_R1_" and "_R2_" in filename to pair
...
#Get a list of read1 and read2 FASTQ files in the input directory
read2_files = [filename for filename in os.listdir(input_directory) if filename.endswith('.fastq.gz') and "_R2_" in filename]
read1_files = [filename for filename in os.listdir(input_directory) if filename.endswith('.fastq.gz') and "_R1_" in filename]

#Pair and align reads using Bowtie2
def align_reads(read1_filename, read2_filename):  
    read2_file_path = os.path.join(input_directory, read2_filename)
    read1_file_path = os.path.join(input_directory, read1_filename)      

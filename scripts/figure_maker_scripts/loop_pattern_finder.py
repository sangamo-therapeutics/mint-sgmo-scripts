    ...
    for res1 in residue_list:
        for res2 in residue_list:
            ...
            if dipeptide_frequency != 0:
                cumulative_value += dipeptide_frequency * math.log(
                    dipeptide_frequency / (res1_frequency * res2_frequency), 2)
...
if __name__ == "__main__":
    ...
    for pos1 in range(peptide_length):
        for pos2 in range(peptide_length):
            if pos2 > pos1:
                ...
                for dipeptide in dipeptide_list:
                    dipeptide_frequency_dict[(pos1, pos2)][dipeptide] = float(
                        dipeptide_dict[(pos1, pos2)][dipeptide]) / float(total_peptides)
                                ...
                                for res3 in residue_list:
                                    for res4 in residue_list:
                                        ...
                                        if tetrapeptide_dict[(pos1, pos2, pos3, pos4)][tetrapeptide] >= 1:
                                            formatted_tetrapeptide = format_tetrapeptide(tetrapeptide, pos1, pos2, pos3,
                                                                                         pos4, peptide_length)
    ...
    if len(plot_peptide_list) > 0:
        ...
        ww_logo.ax.text(peptide_offset, 4.5, '%s' % truncated_filename, fontsize=12)
        ww_logo.ax.text(peptide_offset, -0.3,
                        '%d peptides total: top %d plotted' % (len(sorted_enrichment_list), len(plot_peptide_list)),
                        fontsize=9)

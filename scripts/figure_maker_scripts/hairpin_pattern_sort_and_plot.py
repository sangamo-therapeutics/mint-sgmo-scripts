...
if __name__ == "__main__":
    ...
    rounds = 0
    while len(not_in_motif_list) > 0 and motif_seed != not_in_motif_list[-1]:
    ...
    while motif_pos < motif_count:
        if len(motif_dict[motif_pos]) >= 5:
            ...
            unrepresented_motif_list = []
            for item in motif_dict[motif_pos]:
                unrepresented_motif_list.append(sorted_pattern_list[item][0])
            output_motif_num += 1
            motif_peptides = []
            peptide_pattern_score_list = []
            peptides_already_used = []
            for peptide in peptide_list:
                ...
                if include_in_motif_list == 1:
                    ...
                    if peptide not in peptides_already_used:
                        peptide_pattern_score_list.append((peptide, pattern_match_score))
                        peptides_already_used.append(peptide)
            sorted_peptide_pattern_score_list = sorted(peptide_pattern_score_list, key=lambda x: x[1], reverse=True)
            for item in sorted_peptide_pattern_score_list:
                peptide = item[0]
                pattern_match_score = item[1]
                new_pattern = ''
                for item in motif_dict[motif_pos]:
                    if pattern_match(peptide, sorted_pattern_list[item][0]) == 1:
                        if len(unrepresented_motif_list) > 0:
                            if sorted_pattern_list[item][0] in unrepresented_motif_list:
                                new_pattern += '%s\t' % sorted_pattern_list[item][
                                    0]  # show pattern if first time pattern matches a peptide
                                unrepresented_motif_list.remove(sorted_pattern_list[item][0])
            ...
            for pos in range(peptide_length):
                for res in residue_list:
                    residue_pos_prob_dict[res][pos + peptide_offset] = float(residue_pos_dict[res][pos]) / float(
                        len(motif_peptides))
            ...
            for pos in range(peptide_length):
                ...
                for res in residue_list:
                    IC_scaled_residue_pos_prob_dict[res][pos + peptide_offset] = residue_pos_prob_dict[res][
                                                                                     pos + peptide_offset] * IC_value
                    # print(output_line)
            ...
            ww_logo = logomaker.Logo(df,
                                     ...
                                     width=.9,
                                     figsize=[8.0, 8.0])
            ww_logo.style_single_glyph(c='A', p=315, color=[0.85, 0.85, 0.85])
            ww_logo.style_single_glyph(c='G', p=317, color=[0.85, 0.85, 0.85])
            ww_logo.style_single_glyph(c='R', p=319, color=[0.85, 0.85, 0.85])
            ww_logo.style_single_glyph(c='K', p=320, color=[0.85, 0.85, 0.85])
            ww_logo.style_single_glyph(c='Y', p=324, color=[0.85, 0.85, 0.85])
            ...
            ww_logo.ax.text(peptide_offset, 4.5, '%s - motif %d' % (truncated_filename, output_motif_num - 1),
                            fontsize=12)
            ww_logo.ax.text(peptide_offset, -0.3,
                            '%d peptides total: %d plotted' % (len(peptide_list), len(motif_peptides)), fontsize=9)
